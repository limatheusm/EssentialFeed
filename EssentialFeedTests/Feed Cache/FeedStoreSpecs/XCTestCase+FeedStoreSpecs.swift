...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), to sut: FeedStore) -> Error? {
        ...
        sut.insert(cache.feed, timestamp: cache.timestamp) { receivedResult in
            if case let .failure(error) = receivedResult { insertionError = error }
            exp.fulfill()
        }
    ...
    func deleteCache(from sut: FeedStore) -> Error? {
        ...
        sut.deleteCachedFeed { receivedResult in
            if case let .failure(error) = receivedResult { deletionError = error }
            exp.fulfill()
        }
